...
public class Spider extends Monstre {
    ...
    protected boolean call_;

    protected int cpt_;

    protected int cpt_flee_;

    protected int cpt_rdm_;
     ...
     * @param coord
     */
    public Spider(String type, int health, int direction, boolean under_attack,
            boolean enemy_face, boolean call, Coordinates coord) {
    ...
    public Spider() {
        super("spider", 8, 1, false, false, new Coordinates(0, 0));
        call_ = false;
        cpt_ = 5;
        cpt_rdm_ = 3;
        cpt_flee_ = 2;
    ...
    public void draw(Interface inter) {
        inter.getG2_().drawImage(
                getDemoImage(
                        "/Sprites/characters/" + getType() + "/spider.png",
                        inter), print_x_, print_y_, inter);
    }

    public void draw_med(Interface inter) {
        inter.getG2_().drawImage(
                getDemoImage("/Sprites/characters/" + getType()
                        + "/spider_2.png", inter), print_x_, print_y_, inter);
    }
    ...
    public void flee(Game_screen inter, int len, int larg) {
        Element e = null;
        int i, j;
        i = inter.getHeros_().getCoord().get_x() - ((coord_.get_x()) * 16);
        j = inter.getHeros_().getCoord().get_y() - ((coord_.get_y()) * 16);
